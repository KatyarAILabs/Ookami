...
Thanks for helping. Forge aims to be infrastructure people can run for years, so we value small, tested changes.
...
uv run python scripts/gen_config_reference.py      # after changing src/forge/config.py
```
...
- **The schema is the source of truth.** A new `forge.yaml` field needs a `description=`, a default (unless
  it truly must be set), validation, and a regenerated `docs/reference/forge-yaml.md`. CI checks this.
- **Unknown config is an error, never ignored.**
- **Forge doesn't judge correctness.** Evaluators come from users. Don't add built-in scores that measure
...
Open an issue with:
- your `forge.yaml`, with secrets removed;
