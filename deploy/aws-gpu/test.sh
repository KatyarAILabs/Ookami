#!/bin/bash
# Runs inside the vllm/vllm-openai container on the GPU instance. Exercises everything Forge does today.
set -uo pipefail
...
step validate
check forge validate -f forge.yaml

step "serve the base model with vLLM behind the gateway"
check forge up -f forge.yaml --timeout 900
check forge status -f forge.yaml
check forge down -f forge.yaml

step "train (TRL LoRA) -> gate vs base (vLLM multi-LoRA)"
forge train router -f forge.yaml; echo "train exit $?"
forge jobs -f forge.yaml
forge models -f forge.yaml
cat .forge/reports/router/*.md 2>/dev/null

step "promote and serve the trained version"
check forge promote router -f forge.yaml
check forge up -f forge.yaml --timeout 900
python3 - <<'PY' || fail=1
