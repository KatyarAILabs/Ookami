"""The Evaluator seam: how customers tell Ookami what "good" means. Ookami never decides correctness itself.

Two shapes:
  RowEvaluator      score(example, output) -> Score. Ookami produces the output (calls the model, or reads a
...
def evaluator(name: str | None = None, version: str = "1") -> Callable[[Callable], Callable]:
    """Mark a customer function as a Ookami evaluator:

        @evaluator(name="task_check", version="2")
        def score(example, output) -> Score: ...
    """
    def wrap(fn: Callable) -> Callable:
        fn.ookami_name = name or fn.__name__
        fn.ookami_version = version
        return fn
    return wrap


def load_ref(ref: str, base_dir: Path) -> Callable:
    """Import `path/to/file.py:function`, relative to ookami.yaml."""
    ...
    mod_name = f"ookami_user_{abs(hash(str(path)))}"
