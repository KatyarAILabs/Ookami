...
`ookami train` enqueues and starts a worker if none is running. The worker drains the queue one job at a
...
def ensure_worker(cfg: Config) -> None:
    ...
    with open(log, "ab") as out:
        subprocess.Popen([sys.executable, "-m", "ookami.cli", "_worker", "-f", str(cfg.path)],
    ...
    try:
        ...
        with open(d / "train.log", "ab") as out:
            ...
            proc = subprocess.Popen(argv, stdout=out, stderr=subprocess.STDOUT, stdin=subprocess.DEVNULL,
                                    env={**os.environ, "OOKAMI_JOB": str(d / "job.json")})
