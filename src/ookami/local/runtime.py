"""The local backend: `ookami up` / `ookami down` / `ookami status` on one machine.
...
class UpError(RuntimeError):
    """ookami up/down/status could not do what was asked; the message says why."""


def storage_dir(cfg: Config) -> Path:
    if not cfg.platform:
        raise UpError("ookami.yaml needs a Platform document")
...
HOOKS = "ookami.gateway.litellm_hooks"
...
def litellm_bin() -> str | None:
    """Prefer the LiteLLM installed next to ookami (ookami[gateway]) over whatever is on PATH."""
...
def up(cfg: Config, timeout: float = 900.0, log=print) -> list[Service]:
    ...
    if plat.backend != "local":
        raise UpError(f"ookami up runs the local backend; backend {plat.backend} is not implemented yet")
    running = [s for s in read_state(cfg) if alive(s.pid)]
    if running:
        raise UpError(f"already up ({', '.join(s.name for s in running)}); run ookami down first")
    ...
    try:
        ...
        if gw.mode == "managed":
            ...
            if gw.command:
                ...
                exe = litellm_bin()
                if not exe:
                    raise UpError("the managed gateway needs LiteLLM: pip install 'ookami[gateway]'")
