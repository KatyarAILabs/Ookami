"""Hooks loaded by the managed LiteLLM gateway (see local/runtime.py). They run inside the gateway process.
  ...
  custom_auth: forge.gateway.litellm_hooks.user_api_key_auth
  callbacks:   [forge.gateway.litellm_hooks.usage_logger]

FORGE_STORAGE (set by forge up) points at the storage directory holding registry.db and the master key.
"""
...
def store() -> KeyStore:
    ...
    if _store is None:
        _store = KeyStore(Path(os.environ["FORGE_STORAGE"]))
